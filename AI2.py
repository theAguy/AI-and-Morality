...
if st.button("Send your satisfaction"):
    st.write("Thank you")
...
if st.checkbox('    Press to extend the discussion'):
    st.markdown("<p style='text-align: right;'>"
                """
בפרוייקט שלנו ביצענו סקר עליו ענו כ-300 איש. מטרת הסקר הינה ליצור תשתית נתונים לאלגוריתם אשר אמור לחזות את בחירות משתמשי האתר
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: right;'>"
                """
מאפייני הסקר נבחרו בקפידה לאחר חיפוש והתעמקות בעשרות סקרים שנעשו לאורך השנים במעקב אחר תהליך הבחירות. 
המאפיינים נבחרו כך שהם שומרים על פרטיות הממלא, ממצים כמה שיותר את השונות בין האנשים על מנת לייצר אלגוריתם המלצה טוב שידע לתרגם את המאפיינים אל בחירת המפלגה
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: right;'>"
                """
נשים לב בטבלת הקורלציה שרוב המאפיינים אינם תלויים אחד בשני - למעט מספר ילדים וגיל: בחור/ה צעיר/ה קרוב לוודאי עם פחות ילדים אם בכלל לעומת בחור/ה בוגר/ת עם יותר ילדים. 
נתון זה מעודד שכן כל מאפיין תורם את חלקו לתהליך ולא מהווה חיזוק של מאפיין אחר. 
(ראה ערך מטריצת קורלציה, האלכסון מורכב מאחדות שכן הוא מכל נתון לעצמו)
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: right;'>"
                """
הרשומות שקיבלנו אינן מתפלגות נורמלית, אין מספר שווה או לפחות קרוב מכל סוג רשומה, דבר שעלול ליצור הטייה בתוצאות שכן אלגוריתם הלמידה שלנו רואה יותר דוגמאות מסוג אחד ופחות מסוג אחר.
לדוגמא 90% מהעונים על הסקר היו יהודים, מספר ההצבעות לרשימה המשותפת היו מועטות מאוד וכו׳ (כאשר מספר היהודים באוכלוסייה עומד על כ-75%)
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: right;'>"
                """
                בנוסף, נשים לב לעובדה שכל המאפיינים נבחרים מתוך רשימה קצרה של אופציות, או סקאלה מצומצמת למעט בחירת המאפיין ״דרג עצמך על המפה הפוליטית בין 0 ל-100״ אשר מסמל בחירה בין ימין לשמאל והוא המשפיע ביותר ועובדה זו יכולה להיות משוייכת לכך שמילוי המאפיין הזה ע״י כל בן אדם באשר הוא, הוא סובייקטיבי לחלוטין, בן אדם המרגיש שהוא שמאל מרכז ומסמן 45 לעומת בן אדם אחר שיגדיר עצמו כשמאל מרכז ויסמן 25 כי ככה ״הרגיש לו״ למרות ששניהם באותה הדעה ויצביעו לאותה המפלגה.
דבר זה יגרום לכך ששני ערכים שונים לחלוטין ואף רחוקים אחד מן השני בסדר גודל של חמישית מהסקאלה, ידרשו ערכים זהים מה שיגדיל את השונות מכיוון שאם לדוגמא מישהו סימן שהוא 2 במפה הפוליטית ואין כזו עוד דוגמא האלגוריתם שלנו יושפע ויגדיל את התפלגות הנתונים שלנו. 
מכיוון שכך נתון זה בעל ההשפעה הגדולה ביותר.
אולם, ככל שיהיו לנו יותר תצפיות, וההתפלגות תהיה נורמלית לחלוטין, לנתון זה יהיה משמעות ועזרה בקבלת החלטה של האלגוריתם. כמו כן, אם נצפה בסטיות רבות כתוצאה מפרמטר זה, נוכל להחליש את המשקולת שלו ולתת לו פחות חשיבות בתחזיות עתידיות
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: right;'>"
                """
מכיוון שכמות הנתונים שהצלחנו לאסוף יחסית קטנה, השתמשנו באלגוריתם שנקרא ״יער אקראי״ שמספק לנו תוצאות טובות (נבדק על כמות של כ-100 איש, מתוכם לכ-90% המערכת המליצה את המפלגה שהם באמת מתכננים להצביע עבורה)
    ...
    st.markdown("<p style='text-align: right;'>"
                """
בנוסף, מכיוון שיש מעט תצפיות, החלטנו לקחת את כל התצפיות כסט של אימון, ולא להשאיר חלק מן התצפיות לסט של בדיקה. זה נובע מהעובדה שאם תצפיות ייחודיות מסויימות יפלו מחוץ לסט האימון, סט הבדיקה יהיה מאוד לא מדוייק, ולכן ניקח את כל התצפיות לסט האימון. בהמשך, כשמספר התצפיות ייגדל משמעותית, נוכל לבדוק את מהימנות האלגוריתם על בסיס סט אימון וסט בדיקה
                """
                "</p>", unsafe_allow_html=True)

    st.markdown("<p style='text-align: right;'>"
                """
יער אקראי בהגדרה, כפי ששמו מרמז עליו, מורכב עצי החלטה.
...
בהנחה והיו לנו מאות אלפי רשומות היינו משתמשים באלגוריתמי למידה עמוקה ורשתות נוירונים
    ...
    st.markdown("<h2 style='text-align: right;'>"
                """
?למה להשתמש בלמידה עמוקה                """
                "</h2>", unsafe_allow_html=True)

    st.markdown("<p style='text-align: right;'>"
                """
	:למידה עמוקה יכולה לדגם את האינטראקציות הלא לינאריות בנתונים עם הפעלות לא ליניאריות של פונקציות כגון#
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center;'>"
                """
Leaky\ReLU, Sigmoid, Tanh
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: right;'>"
                """
	:למידה עמוקה יכולה ללמוד ביעילות את גורמי ההסבר הבסיסיים והייצוגים השימושיים מנתוני קלט#
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center;'>"
                """
	מצמצמת את המאמצים בתכנון תכונות בצורה ידנית וסכמטית מראש
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center;'>"
                """
	מאפשרת למערכות המלצה לכלול מידע על תוכן הטרוגני כגון טקסט, תמונות, שמע ואפילו וידאו
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: right;'>"
                """
	למידה עמוקה עובדת מעולה כשמדובר בנתונים רציפים# 
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: right;'>"
                """
	למידה עמוקה כוללת גמישות גבוהה. ישנם כלי למידה עמוקה פופולריים רבים בימינו#
כלים אלה מפותחים בצורה מודולרית ויש להם תמיכה פעילה/קהילתית/מקצועית. המודולריזציה הטובה הופכת את הפיתוח וההנדסה של למידה עמוקה ורשתות נוירונים ליעילים הרבה יותר
                """
                "</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: right;'>"
                """
:אנו מגישים שתי הצעות שלנו למימוש מערכת המלצה באמצעות למידה עמוקה בהנחה והיה לנו יותר דאטא
                """
                "</p>", unsafe_allow_html=True)
...
רשת נוירונים 
    ...
    st.markdown("<p style='text-align: right;'>"
                """
היינו ממירים כל רשומה לווקטור מאפיינים (תוצאות הסקר), מנרמלים ערכים בהתאם  ומטפלים בערכים ריקים.
את המפלגה המתאימה לוקטור הקלט היינו מייצגים בעזרת "וואן-הוט וקטור".
גודל שכבת הקלט – כגודל וקטור המאפיינים.
גודל שכבת הפלט – כמספר המפלגות, ופונקציית סופט-מקס.
מחלקים את הווקטורים לאימון ובחינה, זאת בהתחשב בהתפלגות המידע שלנו (יחסים שווים של המפלגות בין הסטים).
לאחר מכן מבצעים טיונינג, ומודדים את עצמנו בעזרת מטריקות– מכיוון שבמקרה הספציפי שלנו הדאטה הוא לא בבאלאנס, לא תמיד זה ככה.
פונקציית ההפסד שנבחר היא קטגורית קרוס-אנטרופי.
כשנרצה לבדוק את עצמנו / להמליץ לאדם חדש – נבצע את אותו התהליך שביצענו באימון ומוקטור התוצאה נבחר את המפלגה עם ההסתברות הכי גבוהה שנקבל
                """
                "</p>", unsafe_allow_html=True)
...
Auto Encoder – Unsupervised Learning
    ...
    st.markdown("<p style='text-align: right;'>"
                """
ארכיטקטורה המיועדת למציאת ייצוג של וקטורים.
שכבות הקלט והפלט – כגודל וקטור המאפיינים.
את הרשת נאמן לשחזר את הקלט. קלט = פלט.
נבחר כפונקציית  הפסד את ממוצע ריבועי הטעויות.
באמצע ישנה שכבה של ״צוואר בקבוק״ ממנה ניקח את הייצוגים שלמדנו.
נאמן את הרשת על סט האימון שלנו.
לאחר מכן נבנה מודל שמורכב מכל השכבות משכבת הפלט ועד שכבת ״צוואר הבקבוק״.
בעזרת המודל החדש נוציא ייצוגים לוקטורים שלנו. 
את הייצוגים נקלסטר בעזרת כל אלגוריתם קליסטור שנרצה – למשל קיימינס.
כשנרצה לבדוק את עצמנו / להמליץ לאדם חדש -
נוציא את הייצוג של וקטור המאפיינים שלו עם המקודד שלנו.
ישנן כמה אפשרויות
                """
                "</p>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='text-align: center;'>"
                """
	להריץ את הקיימינס מחדש יחד עם הדוגמאות שאנחנו יודעים את התשובות שלהן, ולקחת את המפלגה הפופולרית ביותר בקלסטר של הווקטור שבדקנו ( חצי מפוקח)#
                """
                "</p>", unsafe_allow_html=True)

    st.markdown("<p style='text-align: right;'>"
                """
לשני המודלים שהצענו נוכל לעזור לפונקציית ההפסד שלהם על מנת לדייק את המודל שלנו ולהוסיף את מדד שביעות הרצון לאחר מילוי המאפיינים שיפעל ביחס הפוך, שביעות רצון נמוכה מן התוצאה תוביל לעונש גדול יותר ושינוי מאסיבי יותר של המשקולות במודל.
שביעות רצון גבוהה תוביל לעונש קטן יותר.
עצירה של תהליך האימון תתקבל כאשר ההפסד לא ישתנה מעבר לרף מסוים שאנו נגדיר מראש
                """
                "</p>", unsafe_allow_html=True)
